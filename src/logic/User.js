...
export async function registerUser(email, password, name, myEvents) {
  ...
  if (result.succes) {
    const profileResult = createNewWithId(
      PROFILES_COLLECTION,
      { email, name, myEvents },
      result.id
    );
...
export async function editProfile(PROFILES_COLLECTION, id, userProfile) {
  const result = await updateElement(PROFILES_COLLECTION, id, userProfile);
  console.log("user logic editprofile", result);
